...
from build import *
from pretrain_simsiam import pretrain
...
def pretrain_worker_rcrop(rank, world_size, cfg):
    ...
    local_args.simsiam.encoder = build_model(local_args.model)
    model = build_model(local_args.simsiam)
    ...
    if local_args.fix_pred_lr:
        optim_params = [{'params': model.module.encoder.parameters(), 'fix_lr': False},
                        {'params': model.module.predictor.parameters(), 'fix_lr': True}]
    else:
        optim_params = model.parameters()

    optimizer = build_simsiam_optimizer(optim_params, local_args.op.pretrain)
    ...
    for epoch in range(start_epoch, local_args.run_p.pretrain_epoch + 1):
        ...
        adjust_lr_simsiam(local_args.lr_cfg.pretrain, optimizer, epoch)
...
def pretrain_worker_ccrop(rank, world_size, cfg):
    ...
    for epoch in range(start_epoch, local_args.run_p.pretrain_epoch + 1):
        ...
        if epoch >= local_args.box.warmup_epochs and epoch != local_args.run_p.pretrain_epoch and epoch % local_args.box.loc_interval == 0:
            ...
            all_boxes = update_box(eval_loader, model.module.encoder, len_ds, True if local_args.local_rank == 0 else False, t=local_args.box.box_thresh)  # on_cuda=True
