...
def pretrain_worker_ccrop(rank, world_size, cfg):
    ...
    for epoch in range(start_epoch, local_args.run_p.pretrain_epoch + 1):
        ...
        pretrain_set.use_box = epoch > local_args.box.warmup_epochs
        ...
        if epoch >= local_args.box.warmup_epochs and epoch != local_args.run_p.pretrain_epoch and (
            ...
            all_boxes = update_box(eval_loader, model.module, len_ds, True if local_args.local_rank == 0 else False,
...
def pretrain_worker_ccrop_sc(rank, world_size, cfg):
    print('==> Start rank:', rank)
    local_args = copy.copy(cfg)
    local_args.local_rank = rank % world_size
